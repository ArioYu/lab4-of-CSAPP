...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    if(((op == A_ADD) && (((int)argA < 0) == ((int)argB < 0)) && (((int)val < 0) != ((int)argA < 0)))
       || ((op == A_SUB) && (((int)argA < 0) != ((int)argB < 0)) && (((int)val < 0) == ((int)argA < 0))))
    {
        ovf = TRUE;
    }
...
stat_t nexti(y64sim_t *sim)
{
    ...
    switch (icode) {
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        if(!set_long_val(sim->m, get_reg_val(sim->r, regB)+imm, get_reg_val(sim->r, regA)))
        {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, get_reg_val(sim->r, regB)+imm);
            return STAT_ADR;
        }
        sim->pc = next_pc;
        break;
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        if(!get_long_val(sim->m, get_reg_val(sim->r, regB)+imm, memory))
        {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, get_reg_val(sim->r, regB)+imm);
            return STAT_ADR;
        }
      ...
      case I_JMP: /* 7:x imm */
        ...
        break;
      case I_CALL: /* 8:x imm */
        nextrsp = get_reg_val(sim->r, REG_RSP)-8;
        if(!set_long_val(sim->m, nextrsp, next_pc))
        {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, nextrsp);
            set_reg_val(sim->r, REG_RSP, nextrsp);
            return STAT_ADR;
        }
        set_reg_val(sim->r, REG_RSP, nextrsp);
        sim->pc = imm;
      ...
      case I_RET: /* 9:0 */
        nextrsp = get_reg_val(sim->r, REG_RSP)+8;
        if(!get_long_val(sim->m, get_reg_val(sim->r, REG_RSP), memory))
        {
      ...
      case I_PUSHQ: /* A:0 regA:F */
        nextrsp = get_reg_val(sim->r, REG_RSP)-8;
        if(!set_long_val(sim->m, nextrsp, get_reg_val(sim->r, regA)))
        {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, nextrsp);
            set_reg_val(sim->r, REG_RSP, nextrsp);
            return STAT_ADR;
